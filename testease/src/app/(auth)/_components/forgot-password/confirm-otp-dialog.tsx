...
const ConfirmOTPDialog: React.FC<ConfirmOTPProps> = ({ email, isOpen, onClose, onSuccess }) => {
  ...
  const sendOtp = async () => {
    try {
      ...
      const response: ApiResponse<null> = await wretch('/api/access/forgot-password/send-otp').post({ email }).json();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      }> = await wretch('/api/access/forgot-password/confirm-otp').post({ email, otp }).json();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className='max-w-[300px] md:max-w-[425px]'>
        ...
        <DialogFooter>
          <Button type='button' variant={'outline'} onClick={sendOtp} disabled={isResendDisabled || isLoading}>
            Resend OTP
          </Button>
