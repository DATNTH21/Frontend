...
const OtpDialog: React.FC<OtpDialogProps> = ({ email, isOpen, onClose }) => {
  ...
  const handleSubmit = async () => {
    setIsLoading(true);
    try {
      const response: ApiResponse<null> = await wretch('/api/access/verify/confirm-otp').post({ email, otp }).json();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className='max-w-[300px] md:max-w-[425px]'>
        ...
        <DialogFooter>
          <Button type='button' variant={'outline'} onClick={sendOtp} disabled={isResendDisabled}>
            Resend OTP
          </Button>
