...
const ResetPasswordDialog: React.FC<ResetPasswordProps> = ({ infoForgotPassword, isOpen, onClose }) => {
  ...
  const handleSubmit = async (data: { password: string; confirmPassword: string }) => {
    ...
    try {
      const response: ApiResponse<null> = await wretch('/api/access/forgot-password/reset-password')
        .post({
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className='max-w-[300px] md:max-w-[425px]'>
        <DialogHeader>
          ...
          <DialogDescription>Please enter your new password to reset your password.</DialogDescription>
        </DialogHeader>
